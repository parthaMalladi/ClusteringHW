...
#1. Run this code first bu5t before runnung make sure to update the paths to the data files(yum_flavor, yum_tfidf.pkl, yummly_clean.pkl, yummly_ingr.pkl, yummly_ingrX.pkl and yummly.pkl)
#2.numpy-->1.23.5, pandas-->1.5.3 (highly important to avoid data fregmentation errors ), matplotlib-->3.6.2,seaborn-->0.12.2, scipy-->1.9.3, nltk-->3.8.1(highly importtant),scikit-learn-->1.1.3(highly important), bokeh-->3.2.0.
#3. The code runs on python 3.9 or 3.10 version. The code might show potential errors for version 3.12 or other versions. If using Python 3.12, make sure to update TSNE, bokeh, pandas, and nltk.
#After running this code the following files will be created:
#Creates pickled files:
#yummly_clean.pkl (cleaned recipes)
#yummly_ingr.pkl (processed ingredients)
#yummly_ingrX.pkl (ingredient matrix)
#yum_flavor.pkl (flavor matrix)
#yum_tfidf.pkl (TF-IDF transformed flavor data)
import numpy as np
import pandas as pd
...
import re
import pickle  # cPickle is no longer needed in Python 3
import itertools
import nltk
nltk.download('punkt')
...
def split_ingr(x):
    ...
    cleanlist = []
    lst = x.strip('[]').split(',')
    cleanlist = [' '.join(word.lower() for word in re.sub('[^a-zA-Z ]', ' ', item).split()) for item in lst]
...
def cleanup_ingredients(ingr, df, col):
    ...
    long_ingredients = list(filter(lambda x: ' ' in x, ingr))
...
# Tweak dictionary for ingredient matching
...
def count_missing(lst, df_dic):
    cnt = 0
    for item in lst:
        if item in df_dic or 'salt' in item.split() or 'sugar' in item.split() or 'water' in item.split() or 'oil' in item.split():
            cnt += 1
    return len(lst) - cnt
...
# Project recipes from ingredient matrix to flavor matrix using flavor network
...
# Normalize flavor matrix using tfidf method
def make_tfidf(arr):
    arr2 = arr.copy()
    N = arr2.shape[0]
